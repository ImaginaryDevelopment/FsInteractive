...
//    extern IntPtr private SendMessageW(IntPtr hWnd, int Msg, IntPtr wParam)
    [<DllImport("user32.dll")>]
    extern void private keybd_event(byte _bVk, byte _bScan, UInt32 _dwFlags, int _dwExtraInfo);
...
type VolumeUpEffect() =
    interface ISideEffect with
        member __.Description = "Turn the Volume up when run"
        member __.Execution _ =
            VolumeEffects.volumeUp()
            printfn "Turned volume up"
        member __.Name = "VolumeUp"