...
module Parser =  
    ...
    let pexpr = pexpr_ <??> "pexpr failure"
...
//    let pobject = passign
    ...
    let parrayinitializer =
        exprBraceSeq pexpr <??> "parrayinitializer expected {comma delimited exprs}"
//        between (str_ws "{") (str_ws "}") (many pexpr)
        |>> fun args -> ObjectInitializer.ArrayInitialization args
    let ptypedarray = 
...
//    let pobjinitializer,pbojinitializerImpl = createParserForwardedToRef ()
    ...
    let pobjinitializer =
        exprBraceSeq (pidentifier .>> ws .>> str_ws "=" <??> "expected =" .>>. (optional (str_ws "new") >>. pexpr <??> "pbojinitializer.pexpr") <??> "namevaluepair expected k = v") 
    ...
    let pconstructorcall =  // not necessarily pinvoke may not have ()
        optional(str_ws "new ") >>. ptype .>> ws .>>. (opt pconstructorArgs) .>> ws .>>. (opt (attempt parrayinitializer <|> pobjinitializer))
        <??> "expected a constructor call new Object(){} or new String() or new [] {} or new obj[] {}"
        //(name,args),objectInitializers
        |>> fun ((name,args),oi) -> 
            printfn "constructor name=%s" name
            ObjectConstructor(name, defaultArg args List.empty, oi)
...
let singleTableInfo = """new TableInfo
            {
                Name="Payment",
                Schema="dbo",
                Columns = new []
...
//let ex = 
//    Example.program
//    |> FParsec.CharParsers.run pnsscope

//test passignment (sprintf "var x = %s" text)

let runTests() = 
    [
        run pexpr "new ColumnInfo()"
        // anonymous object
        //run pexpr' "(new { Columns = new [] { 1, 23 } })"
        run pexpr' "(new Test{ Columns = new [] { 1, 23 } })"
        run pconstructorcall singleTableInfo
        run pconstructorcall (text.Trim())
    ]   
    |> Seq.mapi (fun i r -> match r with | Success _ -> None | Failure (a,b,c) -> Some (i,a,b,c) )
    |> Seq.choose id
    |> printfn "%A"