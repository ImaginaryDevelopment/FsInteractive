...
module CHelpers =
    ...
    let tryParseDecimal x = match Decimal.TryParse x with | true, x -> Some x | _ -> None
    let tryParseInt x = match Int32.TryParse x with | true, x -> Some x | _ -> None
    ...
    let tryOrLog topic f =
        let f = Action.invoke f 
        try
            f()
        with ex ->
            logEx ex topic
    let tryOrLogFunc topic f =
        let f = Func.invoke f
        try
            f()
            |> Some
