...
module FunctionalHelpersAuto =
    ...
    let cprintn c (msg:string) =
        makePrintFunction c (fun () -> System.Console.WriteLine msg)
...
//    let before (delimiter:string) (x:string) = x.Substring(0, x.IndexOf delimiter)
    ...
    let alterCasing fPrefix fChange (s:string) =
        let prefix = s |> Seq.takeWhile fPrefix
        s
        |> Seq.skipWhile fPrefix
        |> Seq.mapi fChange
        |> Seq.append prefix
        |> String.Concat
    ...
    // doesn't account for leading _
    let toPascalCase s =
        alterCasing (Char.IsLetter >> not) (fun i l -> if i=0 && Char.IsLower l then Char.ToUpper l else l) s

    let toCamelCase s =
        alterCasing (Char.IsLetter >> not) (fun i l -> if i=0 && Char.IsUpper l then Char.ToLower l else l) s
...
module StringPatterns =
    ...
    let (|ParseInt|_|) (x:obj) =
        ...
        | _ -> None
    let (|ParseFloat|_|) (x:obj) =
        match x with
        | :? string as p ->
            let success,value = System.Double.TryParse(p)
            if success then
                Some value
            else None
        | _ -> None
    ...
    let (|ParseDecimal|_|) (s:string) =
        ...
        | true, x -> Some x
        | _ -> None
    let (|RMatch|_|) (pattern:string) (s:string) =
        let r = Regex.Match(s,pattern)
        if r.Success then
            Some r
        else None
...
type FailureInfo =
    ...
    with
        ...
        member x.GetText() =
            ...
            | Text t -> Some t
            | _ -> None
        static member Add fTxt x =
            match x with
            | TxtEx(t,ex) -> TxtEx(fTxt t, ex)
            | Text t -> fTxt t |> Text
            | Ex ex -> TxtEx(fTxt null, ex)
        member x.AddText fTxt = FailureInfo.Add fTxt x
        member x.AddText funcTxt = FailureInfo.Add (Func<_>.invoke1 funcTxt) x


type ChoiceRailway<'t> = Choice<'t,FailureInfo>
module Choice = // https://github.com/fsprojects/FSharpx.Extras/blob/master/src/FSharpx.Extras/ComputationExpressions/Monad.fs
    ...
    let inline bindIsNullOrWhitespace msg =
        ...
        | _ -> Choice2Of2 (FailureInfo.Text msg)
    let inline iter f =
        function
        | Choice1Of2 x -> f x
        | _ -> ()
...
module Security =
    ...
    // not as good of security as the above somewhat
    let insecureSecureString ss =
        let mutable x = null
        decryptSecureString (fun y -> x <- y) ss
        x
...
module Inpc =
    ...
    let triggerPropChanged (event:Event<PropertyChangedEventHandler,PropertyChangedEventArgs>) source name () =
    ...
    // must have a real source (self =/ ) arg for wpf to listen consistently
    let createInpc source event name options defaultValue =
        let inline fNotifier () = triggerPropChanged event source name ()
        match options with
