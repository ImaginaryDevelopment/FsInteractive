...
module Diagnostics =
    ...
    module TimerInternals =
        let logElapsed scopeName (sw:Stopwatch) = logS (Some scopeName) [] (sprintf "  <Elapsed>%A</Elapsed>" sw.ElapsedMilliseconds)
        let makeSwDisposal scopeName (sw:Stopwatch) =
            {
                new System.IDisposable
        ...
        type TimedScopeHandler (scopeName) =
            ...
            let disp = makeSwDisposal scopeName sw
            let pid = neverThrow (fun () -> Process.GetCurrentProcess().Id) |> Option.getOrDefault 0
            do 
                logS (Some scopeName) [ sprintf "pid=\"%i\"" pid ] ""

            member __.MarkElapsed(name) = logElapsed name sw
    ...
    open TimerInternals
    // this must NEVER throw on the start or finish, otherwise it's not trustable for perf timing
    let BeginTimedLogScope scopeName=
        neverThrow (fun () -> 
            let pid = Process.GetCurrentProcess().Id
            let sw = Stopwatch.StartNew()
            logS (Some scopeName) [ sprintf "pid=\"%i\"" pid ] ""
            makeSwDisposal scopeName sw
        )
        |> function
            | Some disp -> disp
            | None -> 
    ...
    let beginDetailedTimedScope scopeName =
        try
